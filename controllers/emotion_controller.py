...
import io
from user_utils import identify_user, save_emotions_to_user
...
def create_emotion_blueprint(db, config):
    ...
    def draw_boxes(img_array, emotions, scale_x, scale_y):
        ...
        for emotion_data in emotions:
            ...
            cv2.circle(img_array, (int(left_eye_x * scale_x), int(left_eye_y * scale_y)), 4, (0, 0, 255), -1)
            cv2.circle(img_array, (int(right_eye_x * scale_x), int(right_eye_y * scale_y)), 4, (0, 0, 255), -1)
            
            identified_user = emotion_data['identified_user']
            emotion_text = f"{emotion_data['dominant_emotion']} ({emotion_data['emotion_confidence']:.2f})"
            if identified_user:
                emotion_text = f"{emotion_text} - {identified_user['name']} {identified_user['last_name']}"
            
            # Calculate the position for the text
            text_size, _ = cv2.getTextSize(emotion_text, cv2.FONT_HERSHEY_SIMPLEX, 1.0, 1)
            text_w, text_h = text_size
            text_x = x
            text_y = y - 10 if y - 10 > 10 else y + 10
                
            # Draw rectangle behind the text
            cv2.rectangle(img_array, (text_x, text_y - text_h - 5), (text_x + text_w, text_y + 5), (255, 0, 0), -1)

            # Put the emotion text on the image
            cv2.putText(img_array, emotion_text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 1)
